...
gulp.task('style:build', function () {
    // gulp.src('resources/components/serch.scss') //Выберем наш main.scss
    gulp.src('resources/main.scss') //Выберем наш main.scss
        .pipe(sourcemaps.init()) //То же самое что и с js
        .pipe(sass()).on('error', sass.logError) //Скомпилируем
        .pipe(prefixer()) //Добавим вендорные префиксы
        .pipe(cssmin()) //Сожмем
        .pipe(sourcemaps.write())
        .pipe(gulp.dest('public/css/')) //И в public
        .pipe(reload({stream: true}));
});
...
var path = {
    ...
    watch: { //Тут мы укажем, за изменением каких файлов мы хотим наблюдать
        html: 'resources/components/**/*.html',
        js: 'resources/components/js/**/*.js',
...
gulp.task('watch', function(){
    ...
    watch(['resources/**/*.scss'], function(event, cb) {
